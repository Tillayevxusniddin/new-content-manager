...
export function useProgress(bookId: string) {
	...
	const setAudioPosition = (seconds: number) =>
		update({ audioPositionSec: clamp(seconds, 0, 3600) })
	const setVideoPosition = (seconds: number) =>
		update({ videoPositionSec: clamp(seconds, 0, 7200) })
